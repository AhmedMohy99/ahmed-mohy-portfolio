...
function ProjectCard({ project, archived = false }: { project: (typeof projects)[number]; archived?: boolean }) {
  return (
    <article className={`project-row group grid gap-7 border-t border-[var(--line)] py-10 md:grid-cols-[minmax(360px,.72fr)_1fr_auto] md:items-center md:py-14 ${archived ? 'archived-project' : ''}`}>
      ...
      <div>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {project.services.slice(0, 4).map((item) => <span key={item} className="tag">{item}</span>)}
        </div>
        <div className="mt-7 flex flex-wrap gap-3">
          <a href={`/work/${project.slug}`} className="inline-flex items-center gap-2 rounded-full border border-[var(--line-strong)] px-4 py-2 text-xs font-medium uppercase tracking-[0.14em] transition hover:border-[var(--bronze)] hover:text-[var(--bronze)]">
            Case study <ArrowUpRight size={14} />
          </a>
          <a href={project.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[var(--fg)] px-4 py-2 text-xs font-medium uppercase tracking-[0.14em] text-[var(--white)] transition hover:opacity-85">
...
export function WorkSection() {
  ...
  return (
    <section id="work" className="container py-28 md:py-44" aria-labelledby="work-title">
      ...
      <div className="mt-14">
        {visibleProjects.length ? visibleProjects.map((project) => <ProjectCard key={project.slug} project={project} />) : (
          <p className="border-t border-[var(--line)] py-12 text-[var(--muted)]">No projects match this filter yet.</p>
        )}
      </div>
