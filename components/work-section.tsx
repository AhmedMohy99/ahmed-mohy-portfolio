...
function matchesFilter(
  ...
  return categories.some(
    (item) =>
      item.toLowerCase().replace('/', '') === target,
  );
}
      ...
      <div className="relative aspect-[16/10] w-full overflow-hidden">
        {failed ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-[radial-gradient(circle_at_50%_35%,rgba(155,128,85,.24),transparent_38%),linear-gradient(135deg,var(--charcoal),#34322d)] p-8 text-center text-[var(--on-charcoal)]">
            ...
            <span className="display mt-4 text-4xl">
              {project.name}
            </span>

            <span className="mt-3 max-w-xs text-xs leading-relaxed text-white/60">
              The homepage preview is temporarily unavailable.
              Open the live website to view the current site.
            </span>
          </div>
        ) : (
          <Image
...
function ProjectCard({
  ...
  return (
    <article
      className={`project-row group grid gap-7 border-t border-[var(--line)] py-10 md:grid-cols-[minmax(360px,.72fr)_1fr_auto] md:items-center md:py-14 ${
    ...
    >
      ...
      <div>
        ...
        <p className="mt-5 max-w-2xl leading-relaxed text-[var(--ink-soft)]">
          {project.description}
        </p>
...
export function WorkSection() {
  ...
  const visibleProjects = filteredProjects.filter(
    (project) => !archivedSlugs.has(project.slug),
  );
  ...
  return (
    <section
      id="work"
      className="container py-28 md:py-44"
    ...
    >
      ...
      {archivedProjects.length > 0 && (
        <div
          className="mt-24 border-t border-[var(--line-strong)] pt-10 md:mt-32 md:pt-12"
          aria-labelledby="archived-title"
        >
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <div className="label text-[var(--bronze)]">
                Archive · Previous client work
              </div>
            ...
            <p className="max-w-md text-sm leading-relaxed text-[var(--muted)]">
              Earlier projects, preserved as case studies
              and live homepage previews. They remain part
              of the work history, but sit outside the
