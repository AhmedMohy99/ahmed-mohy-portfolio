    ...
    <article className={`project-row group grid gap-7 border-t border-[var(--line)] py-10 md:grid-cols-[minmax(360px,.72fr)_1fr_auto] md:items-center md:py-14 ${archived ? 'archived-project' : ''}`}>
      ...
      <div>
        ...
        <div className="mt-7 flex flex-wrap gap-3">
          ...
          <a href={project.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[var(--fg)] px-4 py-2 text-xs font-medium uppercase tracking-[0.14em] text-[var(--white)] transition hover:opacity-85">Open live site <ExternalLink size={13} /></a>
        </div>
      </div>
...
export function WorkSection() {
  ...
  return (
    <section id="work" className="container py-28 md:py-44" aria-labelledby="work-title">
      ...
      <div className="mt-14">
        {visibleProjects.length ? visibleProjects.map((project) => <ProjectCard key={project.slug} project={project} />) : <p className="border-t border-[var(--line)] py-12 text-[var(--muted)]">No projects match this filter yet.</p>}
      </div>
      ...
      {archivedProjects.length > 0 && (
        <div className="mt-24 border-t border-[var(--line-strong)] pt-10 md:mt-32 md:pt-12" aria-labelledby="archived-title">
          ...
          <div className="mt-10 overflow-hidden rounded-[1.5rem] border border-[var(--line)] bg-[var(--panel)] px-5 md:px-8">{archivedProjects.map((project) => <ProjectCard key={project.slug} project={project} archived />)}</div>
        </div>
      )}
    </section>
  );
