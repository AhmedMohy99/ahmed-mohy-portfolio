...
const projectSummary = projects
  .map((project) => `${project.name}: ${project.description} Categories: ${project.category.join(', ')}. URL: ${project.url}.`)
  .join('\n');
...
function faqReply(text: string, lang: Language) {
  ...
  if (['project', 'projects', 'work', 'portfolio', 'مشروع', 'المشاريع', 'أعمال'].some((key) => normalized.includes(key))) {
    return lang === 'ar'
          ...
          input: [
            {
              role: 'system',
              content: [
                `You are ${site.shortName}'s professional portfolio assistant.`,
                `Answer in ${lang === 'ar' ? 'Arabic' : 'English'}.`,
                'Be concise, helpful and accurate.',
                'Never invent client results, prices, technologies, projects or availability.',
                `Contact: ${site.email} | ${site.whatsapp} | ${site.linkedin} | ${site.github} | ${site.instagram}`,
                `Services:\n${serviceSummary}`,
                `Projects:\n${projectSummary}`,
              ].join('\n\n'),
            },
